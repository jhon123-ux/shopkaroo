...
export default function AdminOrdersPage({ 
  ...
  const [selectedCustomer, setSelectedCustomer] = useState<CustomerOrderSummary | null>(null)
  ...
  return (
    <div className="relative font-body">
      ...
      {/* RECURRING CUSTOMERS AGGREGATE */}
      {recurringCount > 0 && (
        <div className="flex items-center gap-6 p-8 rounded-0 bg-[#F0EBF8] border border-[rgba(74,44,110,0.1)] mb-12 shadow-sm animate-fadeIn">
          <div className="w-14 h-14 rounded-0 bg-white flex items-center justify-center shadow-sm border border-[#E8E2D9]">
            <Repeat2 className="w-7 h-7 text-[#4A2C6E]" strokeWidth={2.5} />
          </div>
          <div>
            <p className="text-[14px] font-bold text-[#4A2C6E] uppercase tracking-[3px] font-heading">
              {recurringCount} Recurring Customer{recurringCount !== 1 ? 's' : ''}
            </p>
            <p className="text-[12px] text-[#6B6058] font-bold uppercase tracking-[1px] mt-1 opacity-60">
              Accounts identified with 2 or more historical transactions.
            </p>
          </div>
        </div>
      )}
                  ...
                  <tr key={o.id} className="border-b border-[#FAF7F4] last:border-0 hover:bg-[#FAF7F4]/50 transition-colors">
                    ...
                    <td className="px-8 py-6">
                      <div className="flex flex-col gap-1 items-start">
                        ...
                        {o.user_id && recurringCustomers[o.user_id] && (
                          <RecurringBadge
                            customer={recurringCustomers[o.user_id]}
      ...
      {/* RECURRING CUSTOMER HISTORY MODAL */}
      {selectedCustomer && (
        <CustomerOrdersModal
